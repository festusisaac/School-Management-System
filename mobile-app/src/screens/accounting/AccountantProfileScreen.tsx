...
export default function AccountantProfileScreen() {
    ...
    return (
      <>
        <View style={styles.headerCard}>
          ...
          <Text style={[styles.statusBadge, { color: sc.color, backgroundColor: sc.bg }]}>{profile.status || 'N/A'}</Text>
          <View style={styles.headerMetaRow}>
            ...
            <View style={styles.headerMeta}>
              ...
              <Text style={styles.metaValue}>{profile.department?.name || 'Accounting'}</Text>
            </View>
          </View>
        </View>
        ...
        <Section title="Contact">
          ...
          <Row label="Phone" value={profile.phone} action={profile.phone ? () => Linking.openURL(`tel:${profile.phone}`) : undefined} />
          <Row label="Address" value={profile.address} last />
        </Section>

        <Section title="Employment">
          <Row label="Designation" value={profile.designation?.name} />
          <Row label="Type" value={profile.employmentType} />
          <Row label="Monthly Basic" value={money(profile.basicSalary)} />
          <Row label="Joined" value={fmtDate(profile.dateOfJoining)} last />
        </Section>

        <Section title="Bank Account">
          <Row label="Bank" value={profile.bankName} />
          <Row label="Account No" value={profile.accountNumber} last />
        </Section>
          ...
          return (
            <View key={p.id} style={styles.payCard}>
              <View style={styles.payHeader}>
                <Text style={styles.payPeriod}>{MONTHS[(p.month || 1) - 1] || 'UNK'} {p.year || ''}</Text>
                <Text style={[styles.payStatus, paid ? { color: COLORS.success, backgroundColor: '#dcfce7' } : { color: COLORS.warning, backgroundColor: '#fef3c7' }]}>
                  {p.status || 'Pending'}
                </Text>
              </View>
  ...
  return (
    <AccountingLayout activeTab="Profile">
      <View style={styles.container}>
        ...
        <View style={styles.tabBar}>
          ...
          <TouchableOpacity style={[styles.tabBtn, tab === 'salary' && styles.tabBtnActive]} onPress={() => setTab('salary')}>
            <Text style={[styles.tabText, tab === 'salary' && styles.tabTextActive]}>Salary</Text>
          </TouchableOpacity>
        </View>
...
function Row({ label, value, last, action }: { label: string; value?: string; last?: boolean; action?: () => void }) {
  return (
    <View style={[styles.row, last && { borderBottomWidth: 0 }]}>
      ...
      <View style={styles.rowValueWrap}>
        ...
        {action && value ? (
          <TouchableOpacity onPress={action} style={styles.rowAction}>
...
const styles = StyleSheet.create({
  ...
  headerCard: { backgroundColor: COLORS.surfaceContainerLowest, borderRadius: 16, padding: 20, alignItems: 'center', borderWidth: 1, borderColor: '#f1f5f9', marginBottom: 16 },
  ...
  statusBadge: { fontSize: 10, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 0.5, paddingHorizontal: 12, paddingVertical: 4, borderRadius: 8, overflow: 'hidden', marginTop: 8 },
  headerMetaRow: { flexDirection: 'row', marginTop: 16, paddingTop: 16, borderTopWidth: 1, borderTopColor: '#f1f5f9', alignSelf: 'stretch' },
  ...
  sectionCard: { backgroundColor: COLORS.surfaceContainerLowest, borderRadius: 14, paddingHorizontal: 16, borderWidth: 1, borderColor: '#f1f5f9' },
  row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f1f5f9', gap: 12 },
  ...
  logoutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#fef2f2', paddingVertical: 14, borderRadius: 12, marginTop: 8 },
  ...
  payStatus: { fontSize: 10, fontWeight: '700', textTransform: 'uppercase', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6, overflow: 'hidden' },
  payNetRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: 10, borderTopWidth: 1, borderTopColor: '#f1f5f9' },
  ...
  emptyCard: { backgroundColor: COLORS.surfaceContainerLowest, borderRadius: 16, padding: 32, alignItems: 'center', borderWidth: 1, borderColor: '#f1f5f9', gap: 8, marginTop: 8 },

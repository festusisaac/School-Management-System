...
const renderVideo = (url: string, className: string, getFullUrl: (path?: string) => string, isHovered: boolean = false) => {
  if (!url) return null;
  ...
  if (youtubeId) {
    ...
    const muteParam = isHovered ? 'mute=0' : 'mute=1';
    return (
      <iframe
  ...
  if (vimeoMatch) {
    ...
    const mutedParam = isHovered ? '0' : '1';
    return (
      <iframe
  ...
  return (
    ...
    >
      {isVideo ? (
        <>
          {isHovered ? (
            renderVideo(item.videoUrl, "w-full h-full object-cover", getFullUrl, true)
          ) : (
          ...
          {filteredItems.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
              {filteredItems.map((item, index) => (
                <GalleryItem key={index} item={item} index={index} getFullUrl={getFullUrl} />
              ))}
            </div>
          ) : (

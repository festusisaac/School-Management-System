...
const StaffDirectoryPage = () => {
    ...
    const handleAddStaff = async () => {
        ...
        try {
            const { nextId } = await staffService.getNextStaffId();
            if (nextId && settings?.staffIdPrefix) {
                setEmployeeIdField(`${settings.staffIdPrefix}${nextId}`);
            }
        } catch (error) {
    ...
    const handleRestore = async (id: string) => {
        ...
        try {
            ...
            toast.showSuccess('Staff member restored successfully!');
            fetchData();
        } catch (error) {
            console.error('Error restoring staff:', error);
            toast.showError('Failed to restore staff member');
        }
    };
    ...
    useEffect(() => {
        ...
        // Password will be generated by backend if left blank
        ...
        <div className="space-y-6 p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    >
                        {showArchived ? <UserCheck size={18} /> : <UserX size={18} />}
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div className="flex justify-end gap-2">
                                                ...
                                                {member.status === 'Inactive' ? (
                                                    <button onClick={() => handleRestore(member.id)} className="p-1 text-green-600 hover:bg-green-50 rounded" title="Restore Staff"><Unlock size={18} /></button>
                                                ) : (
                                                    <button onClick={() => handleDelete(member.id)} className="p-1 text-red-600 hover:bg-red-50 rounded" title="Delete Staff"><Trash2 size={18} /></button>
                                                )}
                                            </div>
                                        </td>
                                    </tr>
                            ...
                            <div className="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-6xl sm:w-full">
                                <form onSubmit={async (e) => {
                                    ...
                                    try {
                                        ...
                                        toast.showError(error.response?.data?.message || 'Failed to save staff member');
                                    } finally {
                                        setSubmitting(false);
                                    }
                                }}>
                                            ...
                                            <div>
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <input type="date" name="dateOfBirth" defaultValue={editingStaff?.dateOfBirth ? new Date(editingStaff.dateOfBirth).toISOString().split('T')[0] : ''} className="w-full border border-gray-300 dark:border-gray-600 rounded-lg p-2 outline-none focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white" />
                                                    </div>
                                                    <div>
                                                                ...
                                                                <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                                                                    ...
                                                                    <p className="text-[10px] text-gray-500 dark:text-gray-400 italic font-medium">
                                                                        {editingStaff 
                                                                            ...
                                                                            : "If left blank, a secure random password will be generated and emailed to the staff member."}
                                                                    </p>
                                                                </div>
                                                            )}
                                                        </div>
                                    ...
                                    <div className="bg-gray-50 dark:bg-gray-800/50 px-6 py-4 flex flex-row-reverse gap-3 border-t dark:border-gray-700">
                                        <button 
                                            type="submit" 
                                            disabled={submitting}
                                            className={`px-8 py-2.5 bg-primary-600 text-white rounded-lg font-bold hover:bg-primary-700 shadow-lg shadow-primary-500/30 transition-all flex items-center gap-2 ${submitting ? 'opacity-70 cursor-not-allowed' : ''}`}
                                        >
                                            {submitting ? (
                                                <>
